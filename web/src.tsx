...
import './style.css';
type Track = { id:string; title:string; artist:string; genre:string; status:string; audioPath:string; durationSeconds:number|null; createdAt:string };
function App() {
  ...
  return <main><header><p>24/7 GENERATED ELECTRONIC MUSIC</p><h1>INFINITE<br/>RADIO</h1></header><section className="controls"><select value={genre} onChange={e=>setGenre(e.target.value)}><option value="DEEP_HOUSE">Deep House</option><option value="MINIMAL_DEEP_TECH">Minimal / Deep Tech</option><option value="DRUM_AND_BASS">Drum &amp; Bass</option></select><button onClick={generate} disabled={busy}>{busy?'QUEUED…':'GENERATE CALIBRATION TRACK'}</button></section><h2>CALIBRATION DECK</h2>{tracks.length===0?<p className="empty">Generate a track, then come back when the worker has finished.</p>:tracks.map(t=><article key={t.id}><div><b>{t.artist} — {t.title}</b><small>{t.genre.replaceAll('_',' ')} · {t.durationSeconds?Math.round(t.durationSeconds/60)+' min':'probing'}</small><audio controls src={'/media/'+t.audioPath.split('/').pop()}/></div><button className="keep" onClick={()=>vote(t.id,'KEEP')}>KEEP</button><button className="reject" onClick={()=>vote(t.id,'REJECT')}>REJECT</button></article>)}</main>;
